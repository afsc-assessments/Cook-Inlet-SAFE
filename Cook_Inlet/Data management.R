...
CIS_Catch_2024 <- read_xlsx(path  = paste0(getwd(),'/','Inseason Catch Data/CIS_data_2024_FINAL_20Nov24.xlsx'))
...
temp.catch <- CIS_Catch_2024 %>% 
  ...
  summarise("Kenai Sockeye salmon"= CATCH_PER_DAY[SPECIES == 'Sockeye salmon']*.583, #Kenai proportion
            "Kasilof Sockeye salmon" = CATCH_PER_DAY[SPECIES == 'Sockeye salmon']*.24, #Kasilof proportion
            "Other Sockeye salmon" = CATCH_PER_DAY[SPECIES == 'Sockeye salmon']*(1-0.583-0.24)) %>% 
...
Kenai.TAC <- TAC.DF$TAC_2024[TAC.DF$SPECIES == "Sockeye salmon"]*0.583 # From google drive Kenai data
Kasilof.TAC <- TAC.DF$TAC_2024[TAC.DF$SPECIES == "Sockeye salmon"]*0.24 # From google drive Kasilof data
Other.TAC <- TAC.DF$TAC_2024[TAC.DF$SPECIES == "Sockeye salmon"]*(1-0.583-0.24) # Remaining tac
...
# Save as csv
write.csv(test3,file = paste0(getwd(),"/Inseason Catch Data/2024 EEZ Catch.csv"),row.names = F)
